...
std::deque<geometry_msgs::PointStamped> Recognizer::HumansDetected()
{
    // Clear deque before using
    HumanDeque.clear();

    for(uint j = 0; j < humans.size(); j++)
    {
        HumanPos.header.seq = humans[j].id;
        HumanPos.header.stamp.sec = (int)humans[j].preTimestamp;
        HumanPos.header.stamp.nsec = (int)((humans[j].preTimestamp - (int)humans[j].preTimestamp) * 1000000000);
        HumanPos.header.frame_id = "\laser_top";   // Human position got and sent in laser frame
        HumanPos.point.x = humans[j].state(1);
        HumanPos.point.y = humans[j].state(2);

        HumanDeque.push_back(HumanPos);
    }
