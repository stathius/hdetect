                             ...
                             const sensor_msgs::LaserScan::ConstPtr &lScan)
{
  ...
  for(uint i = 0; i < scanData.fusion.size() ; i++)
  {
  ...
  for (uint i = 0; i < scanData.labels.size(); i++)
    printf("cluster %03d\tannotation %1d\tprojected %1d\tfusion %1d\n", i, scanData.labels[i], scanData.projected[i],
           scanData.fusion[i]);
...
//  sensor_msgs::LaserScan new_lScan = *lScan;
  ...
  scanData.header.stamp = acquisition_time;
  /*
  ...
  */
  scanData.scan = *lScan;
  scanData.image = *image;
  bag.write("/ClusteredScan", acquisition_time, scanData );
...
void annotator::saveCSV() {
  ...
  int i,j;
  for (i = 0; i < scanData.nclusters; i++)
  {
    //if (projectedClusters[i] == 1) { // IF THIS IS UNCOMMENTED IT ONLY SAVES VISIBLE CLUSTERS
    fprintf(f,"%04d %d %d", scanNo, scanData.fusion[i], scanData.labels[i]);

    for (j = 0; j < scanData.nfeatures; j++) {
      fprintf(f,", %3.20f", scanData.features[i].data[j]);
    }
