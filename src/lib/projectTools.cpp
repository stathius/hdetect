#include "upm/lib/projectTools.hpp"
...
{
  boxSize = (int)(M_TO_PIXELS / sqrt(pow(double(it.x)*1000.0, 2.0) + pow(double(it.y)*1000.0, 2.0)));

  // Compute the upper left and down right corners of the rectangle
  upleft.x = (int)pt2D.x - boxSize / 2;
  upleft.y = (int)pt2D.y - boxSize;
  downright.x = upleft.x + boxSize;
  downright.y = upleft.y + 2 * boxSize;
}
