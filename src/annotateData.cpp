 ...
 * @author Stathis Fotiadis and Bang-Cheng Wang
 * @date 10/10/2012
 */
class AnnotateData
{
    ...
    public:
        /**
          ...
          */
        AnnotateData(std::string bagFile, std::string cameraImageIn, std::string laserScanIn);
...
AnnotateData::AnnotateData(std::string bagFile, std::string cameraImageIn, std::string laserScanIn) :
...
int main(int argc, char* argv[])
{
    ...
    std::string cameraImageIn(argv[2]);
    std::string laserScanIn(argv[3]);

    ros::init(argc, argv, "AnnotateLaser");

    AnnotateData al(bagFile, cameraImageIn, laserScanIn);
